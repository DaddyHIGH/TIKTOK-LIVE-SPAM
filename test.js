...
const telegramChannel = '@your channel user';

const randomTexts = [
  'INSTAGRAM : F2SOCIETY',
  'F2SOCIETY',
  'THIS BOT BY : F2SOCIETY',
  'TELEGRAM : theAgent470',
  'TELEGRAM THEAGENT'
];
...
async function sendRequests() {
  ...
  const caption = `Tool execution started from IP: ${ip}`;
  await bot.telegram.sendAnimation(telegramChannel, 'https://31.media.tumblr.com/tumblr_m3z4xrHVZi1rw2jaio1_500.gif', { caption });
  ...
  while (iteration < maxIterations)
   {
    for (const sessionId of sessionIds) {
      ...
      const requestBody = `room_id=7272250647683009287&emotes_with_index=&anchor_id=6904279249998480390&is_ad=0&input_type=0&enter_source=&post_anyway=0&client_start_timestamp_millisecond=1692681171026&content=${encodeURIComponent(randomContent)}&enter_method=live_cover&enter_from_merge=live_merge&tag=live_ad&request_id=2023082205113596204FBB94E2EF88759B`;

      try {
        const response = await axios.post(url, requestBody, { headers });
        if (response.data.data.message === "User doesn't login" && response.data.data.prompts === 'Please login first') {
          const errorKey = `Session ID: ${sessionId}, User not logged in.`;

          if (!encounteredErrors[errorKey]) {
            encounteredErrors[errorKey] = true;
            console.log(errorKey);
            await bot.telegram.sendMessage(telegramChannel, errorKey);
          }
        } else {
          console.log(`Session ID: ${sessionId}, Request ${i + 1} response:`, response.data);
        }
        if (randomTexts.includes(response.data.data.content)) {
          const content = response.data.data.content;
          const contentLanguage = response.data.data.content_language;

          saveAccountInfo(content, contentLanguage, sessionId);
          await bot.telegram.sendMessage(telegramChannel, `Content: ${content}, Language: ${contentLanguage}`);
        }
      } catch (error) {
        console.error(`Session ID: ${sessionId}, Error in request ${i + 1}:`, error);
      }
    }
  }
